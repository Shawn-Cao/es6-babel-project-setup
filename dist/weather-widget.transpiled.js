...
var WeatherWidgetComponent = function () {
  function WeatherWidgetComponent(city, widgetElement) {
    var _this = this;
    ...
    var apiUrl = 'https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D%22' + encodeURIComponent(this.city) + '%22)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithke';
    var xmlHttp = new XMLHttpRequest();
    xmlHttp.onreadystatechange = function () {
      if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
        var data = _this.parseData(xmlHttp.responseText);
        _this.setDom(data, widgetElement);
      }
    };
  ...
  _createClass(WeatherWidgetComponent, [{
    ...
    key: 'setDom',
    value: function setDom(data, widgetElement) {
